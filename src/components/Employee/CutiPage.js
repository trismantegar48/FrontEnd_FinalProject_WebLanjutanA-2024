...
const CutiPage = () => {
    ...
    const handleCreateCuti = (e) => {
        e.preventDefault();
        if (!username || !tanggalCuti || !tanggalMasuk || !alasan) {
            setError('Please fill in all fields');
            return;
        }
        axios.post('http://localhost:5000/cuti/cuti', { username, tanggal_cuti: tanggalCuti, tanggal_masuk: tanggalMasuk, alasan, status: "menunggu konfirmasi" })
            .then(response => {
                fetchCutiList();
                setUsername('');
                setTanggalCuti('');
